...
async function registerUser(fullName, username, birthday, email, password) {
    try {
        ...
        // Automatically grant admin privileges to users with company email domain
        ...
        const newProfile = new Profile({
            ...
            password: password,
            isMember: true,
            isAdmin: isAdminEmail // Assign admin role based on email domain
        });

        await newProfile.save();
        // Log user registration with role information
        // console.log(`✅ User registered: ${username} ${isAdminEmail ? '(ADMIN)' : '(Regular User)'}`);
        return true;
    } catch (err) {
        console.error("❌ Error registering the profile:", err.message);
        return false;
    }
}
