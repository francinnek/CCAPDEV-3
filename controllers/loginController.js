...
const authController = {
    login: async (req, res) => {
        try {
            ...
            const sessionUsername = sessionUser.username || 'guest';
            const sessionEmail = sessionUser.email || 'unknown';

            // SERVER-SIDE VALIDATION
            if (!username || !password) {
                saveLog(sessionUsername,`❌ User ${sessionEmail} left username and/or password fields empty.`);
            ...
            if (!usernameValidation.isValid) {
                saveLog(sessionUsername, `❌ Invalid username entered by ${sessionEmail}.`);
                return res.render('loginform', { 
            ...
            if (typeof password !== 'string' || password.length === 0) {
                saveLog(sessionUsername,`❌ User ${sessionEmail} left password fields empty.`);
                return res.render('loginform', { 
            ...
            if (password.length > 128) {
                saveLog(sessionUsername,`❌ User ${sessionEmail} entered an invalid password format.`);
                return res.render('loginform', { 
            ...
            if (user) {
                ...
                saveLog(req.session.user.username || sessionUsername,`✅ User ${req.session.user.email || sessionEmail} successfully logged in!`);
                res.redirect(`/admin?user=${encodeURIComponent(user.username)}`); 

            } else {
                saveLog(sessionUsername,`❌ User log in unsuccessful: ${sessionEmail}`);
    ...
    register: async (req, res) => {
        try {
            ...
            if (!registrationValidation.isValid) {
                saveLog(req.session?.user?.username || 'guest', `❌ Invalid registration data entered by ${req.session?.user?.email || 'unknown'}.`);
            ...
            if (registered) {
                saveLog(req.session?.user?.username || username || 'guest', `✅ User ${email} successfully registered!`);
    ...
    showChangePasswordForm: (req, res) => {
        // Query string user if none fallback to logged-in session user
        const username = req.query.user || req.session?.user?.username;
    ...
    changePassword: async (req, res) => {
        try {
            // allow username to be supplied via hidden form input or session for logged-in users
            const {
            ...
            const username = postedUsername || req.session?.user?.username;
            const sessionUser = req.session?.user || {};
            const sessionUsername = sessionUser.username || 'guest';
            const sessionEmail = sessionUser.email || 'unknown';

            saveLog(sessionUsername, `🔐 Password change attempt for ${username}`);

            //SERVER-SIDE VALIDATION
            if (!username || !currentPassword || !newPassword || !confirmPassword) { 
                saveLog(sessionUsername,`❗️ All fields are required for password change ${username}`);
            ...
            if (!passwordValidation.isValid) {
                saveLog(sessionUsername, `❌ Invalid password format for ${username}`);
                return res.render('change-password', {
                    title: 'Change Password - DLSU Airlines',
            ...
            if (newPassword !== confirmPassword) {
                saveLog(sessionUsername, `❌ Password mismatch for user ${username}`);
                return res.render('change-password', {
                    title: 'Change Password - DLSU Airlines',
            ...
            if (!isCurrentPasswordValid) {
                saveLog(sessionUsername, `❌ Incorrect current password for user ${username}`);
                return res.render('change-password', {
                    title: 'Change Password - DLSU Airlines',
            ...
            saveLog(sessionUsername, `✅ Password changed successfully for ${username}`);
...
async function registerUser(fullName, username, birthday, email, password) {
    try {
        ...
        */
        ...
        // Do not rely on `req` here; use username/email passed to the function
        saveLog(username || 'guest', `🔐 User ${email} successfully registered!`);
        return true;
    } catch (err) {
